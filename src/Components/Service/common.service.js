...
export async function GAME_STATUS_UPDATE_API(data, token) {
  try {
    const res = await axios.patch(
      `${baseurl}adminRouter/updateGameStatus`,
      data,
      {
        headers: {
          ...header(token),
...
export async function AVAILABLE_ADMIN_ACCOUNT_DETAILS(data, token) {
  try {
    const res = await axios.get(
      `${baseurl}userRouter/adminAccountsList?userId=${data}`,
      {
        headers: header(token),
      }
    );
...
export async function ACCOUNT_ADD_CREDIT_REQUEST(data, token) {
  try {
    const res = await axios.post(
      `${baseurl}userRouter/addCreditRequest`,
      data,
      {
        headers: {
          ...header(token),
